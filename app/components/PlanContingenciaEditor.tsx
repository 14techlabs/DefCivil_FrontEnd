        ...
        <div className="grid grid-cols-12 min-h-[620px]">
          ...
          <div className="col-span-12 lg:col-span-6 xl:col-span-7 p-7">
            ...
            <div className="space-y-5">
              {section === "identificacao" && <><Field label="Título" value={draft.titulo} onChange={(value) => update("titulo", value)} type="text" /><div className="grid grid-cols-1 md:grid-cols-4 gap-4"><Field label="Versão" value={draft.versao} onChange={(value) => update("versao", value)} type="text" /><label><MetaTag className="mb-1.5 block">SITUAÇÃO</MetaTag><select value={draft.status} onChange={(event) => update("status", event.target.value as PlanconDraft["status"])} className="w-full rounded-lg bg-surface-container-low px-4 py-3 text-sm font-medium text-primary outline-none focus:ring-2 focus:ring-secondary"><option value="rascunho">Rascunho</option><option value="em_revisao">Em revisão</option></select></label><Field label="Início da vigência" value={draft.inicioVigencia} onChange={(value) => update("inicioVigencia", value)} type="date" /><Field label="Fim da vigência" value={draft.fimVigencia} onChange={(value) => update("fimVigencia", value)} type="date" /></div><Field label="Objetivo" value={draft.objetivo} onChange={(value) => update("objetivo", value)} /><Field label="Abrangência" value={draft.abrangencia} onChange={(value) => update("abrangencia", value)} /><Field label="Base legal e normativa" value={draft.baseLegal} onChange={(value) => update("baseLegal", value)} /><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Field label="Responsável pela elaboração" value={draft.elaborador} onChange={(value) => update("elaborador", value)} type="text" /><Field label="Autoridade aprovadora" value={draft.aprovador} onChange={(value) => update("aprovador", value)} type="text" /></div></>}
