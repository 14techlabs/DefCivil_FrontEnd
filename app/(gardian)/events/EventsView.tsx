...
import { Btn, Chip, Icon, MetaTag, SectionHeader, StatusDot, Tab, KPI } from "@/app/components/Primitives";
...
export function EventsView({ eventId }: { eventId?: number }) {
  const router = useRouter();
  const query = useSearchParams();
  const detail = eventId !== undefined;
  const search = query.get("busca") ?? "";
  const statusFilter = query.get("status") ?? "todos";
  const typeFilter = query.get("tipo") ?? "todos";
  const [pageSize, setPageSize] = useState<10 | 25 | 50>(10);
  const requestedPage = Number(query.get("pagina") ?? 1);
  const updateFilter = (key: string, value: string) => {
    const next = new URLSearchParams(query.toString());
    next.set(key, value);
    if (key !== "pagina") next.delete("pagina");
    window.history.replaceState(null, "", `/events?${next}`);
  };
  const listHref = `/events${query.size ? `?${query}` : ""}`;
  ...
  const [selectedId, setSelectedId] = useState<number | null>(eventId ?? null);
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-8 p-8">
      {detail && <Link href={listHref} className="inline-flex items-center gap-2 text-sm font-bold text-secondary"><Icon name="arrow_back" />Voltar para eventos</Link>}
      {!detail && <header>
      ...
      {!detail && <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KPI label="Total de eventos" value={eventos.length} icon="event" />
        <KPI label="Ativos" value={eventos.filter((item) => !isClosed(item.status)).length} icon="pending_actions" />
        <KPI label="Encerrados" value={eventos.filter((item) => isClosed(item.status)).length} icon="event_available" />
        <KPI label="Ocorrências vinculadas" value={eventos.reduce((sum, item) => sum + (item.ocorrencias_count ?? 0), 0)} icon="emergency" />
      </div>}
      {!detail && <section className="grid items-end gap-4 rounded-xl bg-surface-container-low p-4 md:grid-cols-2 xl:grid-cols-[minmax(320px,1fr)_220px_220px_auto]">
        ...
        <label className="min-w-0">
          ...
          <div className="relative">
            <select value={statusFilter} onChange={(event) => updateFilter("status", event.target.value)} className="w-full appearance-none rounded-lg border-none bg-white py-3.5 pl-4 pr-12 text-sm font-bold text-primary focus:ring-2 focus:ring-secondary">
        ...
        <label className="min-w-0">
          ...
          <div className="relative">
            <select value={typeFilter} onChange={(event) => updateFilter("tipo", event.target.value)} className="w-full appearance-none rounded-lg border-none bg-white py-3.5 pl-4 pr-12 text-sm font-bold text-primary focus:ring-2 focus:ring-secondary">
      ...
      {!detail && !loadError && eventos.length === 0 && (
      ...
      {!detail && eventos.length > 0 && (
        <section className="overflow-hidden rounded-xl border border-outline-variant/25 bg-surface shadow-ambient-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1000px] text-left">
              <thead className="bg-surface-container-low">
                <tr className="border-b border-outline-variant/25">
                  {["CÓDIGO / EVENTO", "TIPO", "STATUS", "PERÍODO", "OCORRÊNCIAS", "ZONAS", "AÇÕES"].map((label) => (
                    <th key={label} scope="col" className="px-4 py-3"><MetaTag>{label}</MetaTag></th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filteredEvents.slice((page - 1) * pageSize, page * pageSize).map((item) => (
                  <tr key={item.id} className="border-b border-outline-variant/15 transition-colors last:border-0 hover:bg-surface-container-low">
                    <td className="max-w-[300px] px-4 py-4">
                      <p className="break-words text-[13px] font-bold text-primary">{item.id} · {item.nome}</p>
                      {item.id === eventoVinculadoId && <span className="mt-2 flex items-center gap-1.5"><StatusDot tone="secondary" /><MetaTag className="text-secondary">VINCULAÇÃO ATIVA</MetaTag></span>}
                    </td>
                    <td className="px-4 py-4 text-[11px] font-medium text-on-surface-variant">{TYPE_LABEL[item.tipo]}</td>
                    <td className="px-4 py-4"><Chip tone={statusTone(item.status)}>{statusLabel(item.status)}</Chip></td>
                    <td className="whitespace-nowrap px-4 py-4 text-[10px] font-bold text-on-surface-variant">
                      <p>Início: {formatTimelineDate(item.data_inicio ?? "")}</p>
                      <p className="mt-1">{item.data_fim ? `Fim: ${formatTimelineDate(item.data_fim)}` : "Sem data de término"}</p>
                    </td>
                    <td className="px-4 py-4 text-[12px] font-bold text-primary">{item.ocorrencias_count ?? 0}</td>
                    <td className="px-4 py-4 text-[12px] font-bold text-primary">{item.zonas?.length ?? 0}</td>
                    <td className="px-4 py-4">
                      <Link href={`/events/${item.id}${query.size ? `?${query}` : ""}`} aria-label={`Ver evento: ${item.nome}`} className="inline-flex items-center gap-2 whitespace-nowrap rounded-lg bg-secondary/10 px-4 py-2.5 text-[12px] font-bold text-secondary transition-colors hover:bg-secondary/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary"><Icon name="visibility" className="text-[18px]" />Ver evento</Link>
                    </td>
                  </tr>
                ))}
                {filteredEvents.length === 0 && <tr><td colSpan={7} className="py-12 text-center text-[12px] italic text-on-surface-variant">Nenhum evento encontrado para estes filtros.</td></tr>}
              </tbody>
            </table>
          </div>
          <div className="border-t border-outline-variant/20 p-3">
            <PaginationControls
              pagination={{ pagina: page, total_paginas: totalPages, quantidade_por_pagina: pageSize, total_objetos: filteredEvents.length }}
              onPageChange={(nextPage) => updateFilter("pagina", String(nextPage))}
              onPageSizeChange={(size) => { setPageSize(size); updateFilter("pagina", "1"); }}
              showPageSize={false}
            />
          </div>
        </section>
      ...
      {detail && !loadError && !evento && <p className="card-tonal p-6 text-sm text-on-surface-variant">Evento não encontrado ou indisponível para sua entidade.</p>}
      {detail && evento && (
          ...
          <div className="card-tonal p-7 shadow-ambient-sm">
            <div className="grid items-start gap-6 md:grid-cols-[minmax(0,1fr)_280px]">
              <div className="min-w-0">
                ...
                <h1 className="font-headline text-3xl font-black tracking-tighter text-primary">{evento.nome}</h1>
                <div className="mt-3 flex flex-wrap items-center gap-2"><Chip tone={statusTone(evento.status)}>{statusLabel(evento.status)}</Chip><Chip tone="neutral">{TYPE_LABEL[evento.tipo]}</Chip>{eventoVinculadoId === evento.id && <Chip tone="secondary">Vinculação ativa</Chip>}</div>
                <p className="mt-3 text-xs text-on-surface-variant">{formatTimelineDate(evento.data_inicio ?? "")} · {evento.data_fim ? formatTimelineDate(evento.data_fim) : "Sem data de término"} · {evento.ocorrencias_count ?? 0} ocorrências · {evento.zonas?.length ?? 0} zonas</p>
